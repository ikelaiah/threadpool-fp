...
See the [task API](ThreadPool.Tasks-API.md) and
[`TaskCoordination`](../examples/TaskCoordination/) example for the complete
contract.

For real workflow patterns, see
[`CoordinatedFileBackup`](../examples/CoordinatedFileBackup/) for observation,
failure policy, and pending cancellation, and
[`ParallelLogAnalyzer`](../examples/ParallelLogAnalyzer/) for efficient range
